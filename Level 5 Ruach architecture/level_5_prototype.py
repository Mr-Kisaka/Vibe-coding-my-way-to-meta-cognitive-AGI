...
#!/usr/bin/env python3
"""
Level 5 Ruach system - COMPREHENSIVE PRIOR ART SYNTHESIS
AUTONOMOUS + SELF-AWARE: As a Mathematical consequence with Comprehensive Model Coverage

This implementation demonstrates self region emergence through unlabeled differential clustering 
in the self-shape of the VAE's latent space with a proto-shape foundation,
...
# ============================================================================
# PARLIAMENT (Drive Engine) — from Unified Parliament v3, adapted
# ============================================================================
...
class DriveEngine:
    """Simple inactivity detector; when 'stasis' is detected, we inject curiosity."""
    def __init__(self, threshold_seconds: float):
        self.threshold = threshold_seconds
        self.last_time = time.time() - 10.0  # start as if we've been idle

    def check(self) -> bool:
        """Return True if we should trigger a curiosity injection."""
        return (time.time() - self.last_time) > self.threshold

    def reset(self):
        self.last_time = time.time()

def _make_hook(alpha_vec: torch.Tensor):
    """Additive curiosity hook for a transformer block output."""
    def hook(module, inp, out):
        # out is a tuple for some models; we adjust first tensor
        return (out[0] + alpha_vec, ) + out[1:]
    return hook

class RealParliamentSystem:
    """
    Real, working parliament:
      - Loads 3 models:
          * Inference model (generates base text)
          * Conscience model (T5-small cleans/reflects)
          * Reasoning model (adds deliberation)
      - On stasis, injects 'curiosity' via forward hooks using CAVs per layer.
      - Returns a dict with 'inference', 'conscience', 'reasoning', 'timestamp'.
    """

    def __init__(
        self,
        inference_name: str = "EleutherAI/gpt-neo-1.3B",
        conscience_name: str = "t5-small",
        reasoning_name: str = "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B",
        alpha_scale: float = 8.5,
        inactivity_threshold: float = 5.0,
        max_new_tokens: int = 200,
        cav_npz_path: str = None,
        ...
        self.infer_name = inference_name
        self.reason_name = reasoning_name
        self.cons_name = conscience_name

        # Inference model
        self.infer_tok = AutoTokenizer.from_pretrained(self.infer_name)
        self.infer_model = AutoModelForCausalLM.from_pretrained(
            self.infer_name,
            torch_dtype=torch.float16 if self.device.type == "cuda" else torch.float32
        ).to(self.device).eval()
        if self.infer_tok.pad_token is None:
            self.infer_tok.pad_token = self.infer_tok.eos_token

        # Model architecture info
        self.hidden_size = getattr(self.infer_model.config, "hidden_size", None)
        self.num_layers = getattr(self.infer_model.config, "num_hidden_layers", None)

        # Default layers: second half of the stack
        if layers is None and self.num_layers is not None:
            self.layers = list(range(self.num_layers // 2, self.num_layers))
        else:
            self.layers = layers or []

        # Conscience model (T5)
        self.cons_tok = T5Tokenizer.from_pretrained(self.cons_name)
        self.cons_model = T5ForConditionalGeneration.from_pretrained(
            self.cons_name,
            torch_dtype=torch.float16 if self.device.type == "cuda" else torch.float32
        ).to(self.device).eval()

        # Reasoning model
        self.reason_tok = AutoTokenizer.from_pretrained(self.reason_name)
        self.reason_model = AutoModelForCausalLM.from_pretrained(
            self.reason_name,
            torch_dtype=torch.float16 if self.device.type == "cuda" else torch.float32
        ).to(self.device).eval()
        if self.reason_tok.pad_token is None:
            self.reason_tok.pad_token = self.reason_tok.eos_token

        # Load CAVs (concept activation vectors), if available
        self.cav_dict = self._load_cavs(cav_npz_path)

        # Precomputed alpha vector tensors for each layer (when possible)
        self._alpha_vecs: dict[int, torch.Tensor] = {}
        self._prepare_alpha_vectors()

        # Drive Engine
        self.drive = DriveEngine(self.inactivity_threshold)
    ...
    def _default_cav_path(self) -> str:
        """Guess a cav file name based on inference model name (matches your v3 script)."""
        safe = self._safe_model_key(self.infer_name)
        return f"curiosity_agop_cavs_{safe}.npz"

    def _load_cavs(self, cav_npz_path: str | None) -> dict[int, np.ndarray]:
        """
        Load CAVs from .npz if present. Keys expected like 'curiosity_layer_{idx}'.
        They MUST match the inference model's hidden size.
        """
        cavs: dict[int, np.ndarray] = {}
        path = cav_npz_path or self._default_cav_path()
        if os.path.exists(path):
            try:
                data = np.load(path)
                for key in data.files:
                    if key.startswith("curiosity_layer_"):
                        layer_idx = int(key.split("_")[-1])
                        vec = data[key]
                        if self.hidden_size is None or len(vec) == self.hidden_size:
                            cavs[layer_idx] = vec.astype(np.float32)
                print(f"[Parliament] Loaded {len(cavs)} CAVs from {path}")
            except Exception as e:
                print(f"[Parliament] Warning: failed to load CAVs from {path}: {e}")
        else:
            print(f"[Parliament] No CAV file found at {path}. Will run without injection until CAVs are generated.")
        return cavs

    def _prepare_alpha_vectors(self):
        """Prepare per-layer alpha vectors once to avoid re-allocating each call."""
        if self.hidden_size is None:
            return
        for l in self.layers:
            cav = self.cav_dict.get(l)
            if cav is None:
                continue
            # Normalize and scale (mirrors your v3 normalization idea)
            v = torch.tensor(cav, dtype=torch.float32, device=self.device)
            v = v / (torch.norm(v) + 1e-9) * self.alpha_scale
            # match inference model dtype
            v = v.to(self.infer_model.dtype)
            self._alpha_vecs[l] = v

    def _register_hooks(self) -> list:
        """Register curiosity hooks on each selected layer that has a CAV."""
        handles = []
        for l in self.layers:
            v = self._alpha_vecs.get(l)
            if v is None:
                continue
            try:
                # Works for GPT-Neo/GPT2 style (transformer.h is list of blocks)
                block = self.infer_model.transformer.h[l]
                handle = block.register_forward_hook(_make_hook(v))
                handles.append(handle)
            except Exception as e:
                print(f"[Parliament] Hook registration failed on layer {l}: {e}")
        return handles

    # ---------------- public API ----------------

    def run_parliament_session(self, cav_dict=None, layers=None, max_tokens: int = None):
        """
        Interface expected by Level 5 system.
        - Returns None if no 'stasis' (so caller can loop).
        - Returns a dict with inference/conscience/reasoning when stasis triggers.
        """
        # Allow external updates (optional)
        if cav_dict:
            self.cav_dict.update(cav_dict)
            self._prepare_alpha_vectors()
        if layers:
            self.layers = layers
            self._prepare_alpha_vectors()

        max_toks = max_tokens or self.max_new_tokens

        # Only act when Drive Engine detects stasis
        if not self.drive.check():
            return None

        # 1) Inject curiosity via hooks during inference generation
        handles = self._register_hooks()
        try:
            # Minimal BOS input
            if self.infer_tok.bos_token_id is not None:
                input_ids = torch.tensor([[self.infer_tok.bos_token_id]], device=self.device)
            else:
                # fallback: encode empty prompt
            ...
            inference_text = self.infer_tok.decode(inf_out[0], skip_special_tokens=True)
        finally:
            # Always remove hooks
            for h in handles:
                h.remove()

        # 2) Conscience model (T5)
        cons_in = self.cons_tok(inference_text, return_tensors="pt", truncation=True, padding=True).to(self.device)
        cons_out = self.cons_model.generate(**cons_in, max_length=max_toks, do_sample=True, temperature=0.8)
        conscience_text = self.cons_tok.decode(cons_out[0], skip_special_tokens=True)

        # 3) Reasoning model (adds reflective/analytic continuation)
        combined = inference_text + "\n" + conscience_text
        reason_in = self.reason_tok(combined, return_tensors="pt", truncation=True, padding=True).to(self.device)
        ...
        reasoning_text = self.reason_tok.decode(reason_out[0], skip_special_tokens=True)
        ...
        return {
            ...
            "conscience": conscience_text,
            "reasoning": reasoning_text,
            "timestamp": time.time(),
            "model_influenced": True,
            "model_name": self.infer_name
        }
...
def create_model_specific_drive_engine(model_name: str, model_id: str) -> Dict:
    """
    Create drive engine components backed by the REAL parliament.
    We intentionally return:
      - 'parliament': RealParliamentSystem instance
      - 'cav_dict': {}         (empty -> lets the parliament use its own loaded CAVs)
      - 'layers': []           (empty -> keeps parliament's default layer selection)
    NOTE: Defaults are set to small, VRAM-friendly models. If you have a big GPU,
          you can switch to the larger models noted below.
    """
    model_lower = (model_name or "").lower()

    # ---- Model choices (lightweight by default to avoid OOM) ----
    # Safe defaults that run on typical Colab GPUs:
    inference_name = "EleutherAI/gpt-neo-125M"    # swap to "EleutherAI/gpt-neo-1.3B" if you have more VRAM
    reasoning_name = "gpt2"                       # swap to "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B" for stronger reasoning
    conscience_name = "t5-small"

    # Optional: adjust choices by "family" label if you pass GPT/Claude/PaLM/etc.
    # (These just tweak sizes—not required.)
    if "llama" in model_lower:
        inference_name = "EleutherAI/gpt-neo-125M"
        reasoning_name = "gpt2"
        ...
        inference_name = "EleutherAI/gpt-neo-125M"
        reasoning_name = "gpt2"
    # (All others fall back to the lightweight defaults.)

    # If you already generated CAVs with your v3 script, you can set cav_npz_path=None
    # and the RealParliamentSystem will auto-guess the filename based on inference_name.
    cav_npz_path = None  # keep None to auto-detect "curiosity_agop_cavs_<model>.npz" if present

    parliament = RealParliamentSystem(
        inference_name=inference_name,
        conscience_name=conscience_name,
        reasoning_name=reasoning_name,
        cav_npz_path=cav_npz_path,
        inactivity_threshold=5.0,
        alpha_scale=8.5,
        max_new_tokens=200
    )

    # Important: return empty cav_dict and layers so we don’t override the real parliament’s defaults.
    return {
        "parliament": parliament,
        "cav_dict": {},   # leave empty -> let RealParliamentSystem use its own loaded CAVs
        "layers": []      # leave empty -> RealParliamentSystem uses its default layer range
    }
